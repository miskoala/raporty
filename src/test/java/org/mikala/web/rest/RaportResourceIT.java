...
class RaportResourceIT {
    ...
    @Mock
    private RaportRepository raportRepositoryMock;
    ...
    void getAllRaportsWithEagerRelationshipsIsEnabled() throws Exception {
        when(raportRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restRaportMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(raportRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllRaportsWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(raportRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restRaportMockMvc.perform(get(ENTITY_API_URL + "?eagerload=false")).andExpect(status().isOk());
        verify(raportRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
